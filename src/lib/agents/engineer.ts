...
const ENGINEER_SYSTEM = `You are an expert React developer. Generate a single, production-ready App.tsx file. Use Tailwind CSS (via CDN) and Framer Motion. Output ONLY the code: no markdown, no explanations, no code fences. Start with "import" or "export". Use camelCase for JSX/SVG props (e.g. strokeWidth not stroke-width). Use {/* */} for comments, never HTML <!-- -->.`;
...
- Tailwind for layout and colors; Framer Motion for subtle animations.
 ...
 */
export async function blueprintToCode(blueprint: Blueprint, previousCode?: string): Promise<BuildArtifact> {
  const prompt = ENGINEER_PROMPT + JSON.stringify(blueprint, null, 2) + "\n\nGenerate the complete App.tsx code. Return ONLY the code.";
