...
export default function RawCollection() {
  ...
  // For now, collections are always empty - this will be updated later
  const hasSentCards = false;
  const hasReceivedCards = false;
  ...
  return (
    <div className="Layout_collection flex flex-col w-full min-h-screen bg-gradient-to-b from-cardzzz-wine to-cardzzz-blood">
      ...
      <div className="Section_toggle flex flex-col items-center w-full py-[20px] shrink-0 gap-[15px]">
        ...
        {/* Empty State - Sent */}
        {toggleValue === "sent" && !hasSentCards && (
          <p className="BodyText text-cardzzz-cream text-center font-satoshi font-medium text-[14px] leading-normal">
            Your sent dream cards will appear here
          </p>
        )}

        {/* Empty State - Received */}
        {toggleValue === "received" && !hasReceivedCards && (
          <p className="BodyText text-cardzzz-cream text-center font-satoshi font-medium text-[14px] leading-normal">
            Your received dream cards will appear here
          </p>
        )}
      </div>

      {/* Main Content Area */}
      <div className="Section_main_content flex flex-col flex-1 w-full">
        {/* Dream Cards Content - Will be added later */}
        {((toggleValue === "sent" && hasSentCards) || (toggleValue === "received" && hasReceivedCards)) && (
          <div className="Section_cards_content flex flex-col flex-1 w-full">
            {/* Dream cards will be displayed here */}
          </div>
        )}
      </div>
