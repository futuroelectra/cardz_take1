...
import RawActivateModal from "./RawActivateModal";

export default function RawCollectionWithCards() {
  const [isMenuOpen, setIsMenuOpen] = useState(false);
  const [toggleValue, setToggleValue] = useState<"sent" | "received">("sent");
  ...
  return (
    <div className="Layout_collection flex flex-col w-full min-h-screen bg-gradient-to-b from-cardzzz-wine to-cardzzz-blood">
      ...
      {/* Toggle Section - Centered below navbar */}
      <div className="Section_toggle flex justify-center items-center w-full py-[20px] shrink-0">
        <div className="Comp_Toggle flex items-center gap-[8px] p-[4px] rounded-[20px] bg-white/10 backdrop-blur-md border border-white/20">
          <button
            type="button"
            onClick={() => setToggleValue("sent")}
            className={`Comp_Toggle_Option px-[20px] py-[10px] rounded-[16px] font-roundo font-semibold text-[16px] leading-normal transition-all cursor-pointer ${
              toggleValue === "sent"
          ...
          <button
            type="button"
            onClick={() => setToggleValue("received")}
            className={`Comp_Toggle_Option px-[20px] py-[10px] rounded-[16px] font-roundo font-semibold text-[16px] leading-normal transition-all cursor-pointer ${
              toggleValue === "received"
                ? "bg-cardzzz-cream text-cardzzz-accent"
                : "bg-transparent text-cardzzz-cream hover:opacity-80"
            }`}
          >
      ...
      {/* Main Content Area */}
      <div className="Section_main_content flex flex-col flex-1 w-full px-[15px] pb-[30px] overflow-y-auto">
        {/* Dream Cards Grid - Responsive: 4 columns (xl) -> 3 columns (lg) -> 2 columns (md) -> 1 column (mobile) */}
        <div className="Section_cards_grid grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-[20px] w-full max-w-[1400px] mx-auto pt-[20px]">
          {/* Generate 9 mobile preview cards */}
          {Array.from({ length: 9 }).map((_, index) => (
            <button
              key={index}
              type="button"
              onClick={handleCardClick}
              className="Comp_card_preview flex flex-col items-center justify-center shrink-0 cursor-pointer transition-opacity hover:opacity-90"
            >
              {/* Glass container wrapping iPhone */}
              <div className="Comp_editor_container flex items-center justify-center rounded-[30px] bg-white/10 backdrop-blur-md border border-white/20 relative p-[10px] shrink-0" style={{ 
                width: '287px', 
                height: '604.8px',
              }}>
                {/* iPhone 14 screen */}
                <div className="bg-white rounded-[20px] relative shrink-0" style={{ 
                  width: '273px', 
                  height: '590.8px' 
                }}>
                  {/* iPhone content will go here */}
                </div>
              </div>
            </button>
          ))}
