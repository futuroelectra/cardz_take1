...
type LoadingSignupProps = {
  sessionId?: string;
  buildId?: string;
  onNavigateToEditor?: () => void;
};

export default function LoadingSignup({ sessionId, buildId, onNavigateToEditor }: LoadingSignupProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!isFormValid || !sessionId) {
      if (isFormValid && !sessionId) setPhase("accountCreated");
      else if (isFormValid) setPhase("accountCreated");
      return;
    }
    try {
      ...
      await fetch("/api/build/start", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ sessionId, userId: signData.userId }),
      });
  ...
  if (phase === "complete") {
    return (
      <div className="Layout_signup flex flex-col items-center justify-center w-full h-screen bg-gradient-to-b from-cardzzz-wine to-cardzzz-blood overflow-hidden">
        <div className="text-cardzzz-cream font-roundo text-[24px]">complete</div>
      </div>
    );
  }
      ...
      <div className="Section_page_content flex flex-col items-center gap-[15px] w-full max-w-[400px] relative">
        <div className="Section_signup_container flex flex-col items-center gap-[17px] w-full relative">
          <div className="Section_signup flex flex-col items-start gap-[13px] w-full max-w-[400px] relative">
            ...
            <form onSubmit={handleSubmit} className="section_signup_inputs flex flex-col items-start gap-[13px] w-full relative">
              <div className="Comp_Input_Field_Unfilled w-full relative">
                <input type="text" value={formData.name} onChange={(e) => handleInputChange("name", e.target.value)} placeholder="Name" className="PlaceholderText w-full py-[10px] px-[27px] text-cardzzz-cream font-satoshi font-normal text-[14px] leading-normal relative bg-white/10 backdrop-blur-md border border-white/20 rounded-[20px] placeholder:text-cardzzz-muted outline-none focus:border-cardzzz-cream/40 transition-colors" />
              </div>
              <div className="Comp_Input_Field_Unfilled w-full relative">
                <input type="email" value={formData.email} onChange={(e) => handleInputChange("email", e.target.value)} placeholder="Email" className="PlaceholderText w-full py-[10px] px-[27px] text-cardzzz-cream font-satoshi font-normal text-[14px] leading-normal relative bg-white/10 backdrop-blur-md border border-white/20 rounded-[20px] placeholder:text-cardzzz-muted outline-none focus:border-cardzzz-cream/40 transition-colors" />
              </div>
              <div className="Comp_Input_Field_Unfilled w-full relative">
                <input type="password" value={formData.password} onChange={(e) => handleInputChange("password", e.target.value)} placeholder="Password" className="PlaceholderText w-full py-[10px] px-[27px] text-cardzzz-cream font-satoshi font-normal text-[14px] leading-normal relative bg-white/10 backdrop-blur-md border border-white/20 rounded-[20px] placeholder:text-cardzzz-muted outline-none focus:border-cardzzz-cream/40 transition-colors" />
              </div>
              <button type="submit" disabled={!isFormValid} className={`Comp_Button w-full h-[54px] py-[20px] px-[10px] flex items-center justify-center gap-[10px] rounded-[16.168px] border transition-all ${isFormValid ? "bg-cardzzz-cream text-cardzzz-accent border-cardzzz-cream/50 shadow-[0_4px_4px_0_rgba(0,0,0,0.25)] cursor-pointer hover:opacity-90" : "border-cardzzz-cream bg-transparent text-cardzzz-cream cursor-not-allowed"}`}>
                <span className="Label text-center font-roundo font-bold text-[19px] leading-normal relative">create account</span>
              </button>
            </form>
            <div className="T_C_text w-full text-cardzzz-cream text-center font-satoshi font-normal text-[10px] leading-normal relative">
              ...
              <button type="button" className="underline hover:text-cardzzz-cream/80 transition-colors cursor-pointer">privacy policy</button> and{" "}
              <button type="button" className="underline hover:text-cardzzz-cream/80 transition-colors cursor-pointer">terms of service</button>
            </div>
          </div>
        </div>
      </div>
