...
export default function Chat() {
  ...
  useEffect(() => {
    if (sessionId) return;
    const deviceIdFromStorage =
      typeof localStorage !== "undefined" ? localStorage.getItem(STORAGE_KEY_DEVICE_ID) : null;
    const headers: HeadersInit = { "Content-Type": "application/json" };
    if (deviceIdFromStorage) headers["x-device-id"] = deviceIdFromStorage;
    fetch("/api/session", { method: "POST", headers })
      .then((res) => res.json())
      .then((data: { sessionId?: string; deviceId?: string }) => {
        if (data.sessionId) {
          setSessionId(data.sessionId);
          if (typeof sessionStorage !== "undefined") {
            sessionStorage.setItem(STORAGE_KEY_SESSION_ID, data.sessionId);
            if (data.deviceId) sessionStorage.setItem(STORAGE_KEY_DEVICE_ID, data.deviceId);
          }
          if (data.deviceId && typeof localStorage !== "undefined") {
            localStorage.setItem(STORAGE_KEY_DEVICE_ID, data.deviceId);
          }
        }
      })
      .catch(() => {});
  ...
  const handleApprove = useCallback(
    async (messageId: string) => {
      ...
      const newBuildId = data.buildId ?? null;
  ...
  if (currentScreen === "editor" && sessionId && buildId) {
    return <EditorChat sessionId={sessionId} buildId={buildId} />;
  }
